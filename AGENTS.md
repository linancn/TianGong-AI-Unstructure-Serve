...
- **MinerU 文档解析**（`src/routers/mineru_router.py` 等）  
  ...
  - 额外可选字段 `minio_meta` 会在 `save_to_minio=true` 时把传入字符串写入 `meta.txt`（与 `source.pdf` 同目录），返回的 `minio_assets.meta_object` 会指向该文件，便于下游查阅附加元信息。
  - `mineru_minio_utils.build_minio_prefix()` 支持保留 Unicode/中文字符及常见中文标点，但所有空格（含全角空格）都会被统一替换为 `_`，其余不可打印字符也会折叠为 `_` 并清理多余分隔符，避免中文文件名被统一映射到 `mineru/document`；对应校验见 `tests/test_mineru_minio_utils.py`。
